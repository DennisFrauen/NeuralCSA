...
import numpy as np
from data.data_generation import load_data_ihdp

# Compute oracle gammas of simulated SCM for different sensitvity models
if __name__ == "__main__":
    q = 0.5
    np.random.seed(0)
    config_run = utils.load_yaml("/experiments/ihdp/config")
    d_train, d_val, d_test, Y_1_test, Y_0_test, y_mean, y_std = load_data_ihdp(config_run["data"])
